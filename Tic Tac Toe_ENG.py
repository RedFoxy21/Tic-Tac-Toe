...
canvas = tk.Canvas(width=500, height=350)
canvas.pack()
...
def left_click(self):
    ...
    if xL in range(50, 100) and yL in range(50, 100) and x1 == 0:
        ...
        def f1(): # have to do it this way, because it would throw out error about "name 'x' is used prior to global declaration"
            global x1
            x1 = x1 + 1
        f1()
        ...
        print("Click outside the grid |or| Double input") # Don't know how to detect them separately
...
def right_click(self):
    ...
    if xr in range(50, 100) and yr in range(50, 100) and x1 == 0: 
        ...
        print("Click outside the grid |or| Double input")

def reset():
    canvas.delete("txt") # deleting everything with tag="txt"
    global x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25
    x1=x2=x3=x4=x5=x6=x7=x8=x9=x10=x11=x12=x13=x14=x15=x16=x17=x18=x19=x20=x21=x22=x23=x24=x25 = 0 # would really like to simplify this, but I don't know how to...

B = tk.Button(root, text = "Reset", command = reset)
B.configure(width = 8, height=1)
B.place(x=370, y=120) # Reset button

Q = tk.Button(root, text = "Quit", command = root.destroy)
Q.configure(width = 8, height=1)
Q.place(x=370, y=160) # Quit button
...
def grid():
    for i in range(1,6):
        ...
        canvas.create_rectangle(i*50, 250, 50*(i+1), 300) # Tic tac toe grid

grid()

canvas.bind("<Button-1>", left_click) # draws X
canvas.bind("<Button-3>", right_click) # draws O
